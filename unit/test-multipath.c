...

{
	...
	if (ret < 0) {
		connman_error("can't set test data, err=%d", ret);
		goto err;
	}
...
static bool find_rule(uint32_t table_id, const char *src_ip)
{
...
static bool find_route(uint32_t table_id, const char *dst_ip,
...
static bool find_default_route(uint32_t table_id, const char *gw_ip, int ifindex)
{
	struct route_data r = {0};
	r.table_id = table_id;
	r.gw = gw_ip;
